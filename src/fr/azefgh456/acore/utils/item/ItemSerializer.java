package fr.azefgh456.acore.utils.item;
...
public class ItemSerializer {
		...
		if(item.hasItemMeta()) {
			...
			if(itM instanceof EnchantmentStorageMeta) {
				for(Enchantment enchant : ((EnchantmentStorageMeta)itM).getStoredEnchants().keySet()) {
					builder.append(enchant.getName() + regex2 + ((EnchantmentStorageMeta)itM).getStoredEnchants().get(enchant));
	...
	public static ItemStack deserialize(String itemS) {
		...
		ItemStack item = new ItemStack(Material.getMaterial(Integer.parseInt(data[0])), Integer.parseInt(data[1]), Byte.parseByte(data[2]));
		...
		if(data.length >= 6) {
			if(!data[5].equals("-1")) itM.setDisplayName(data[5]);
			
			if(!data[6].equals("-1")) itM.setLore(Arrays.asList(data[6].split(regex2)));
			
			if(itM instanceof EnchantmentStorageMeta) {
				String enchData[] = data[7].split(regex3);
				for(String ench : enchData) {
					String e[] = ench.split(regex2);
					((EnchantmentStorageMeta)itM).addStoredEnchant(Enchantment.getByName(e[0]), Integer.parseInt(e[1]), true);
				}
			}
			
			if(itM instanceof LeatherArmorMeta) {
				String cdata[] = data[8].split(regex2);
				((LeatherArmorMeta)itM).setColor(Color.fromRGB(Integer.parseInt(cdata[0]), Integer.parseInt(cdata[1]), Integer.parseInt(cdata[2])));
			}
			
			if(itM instanceof SkullMeta) ((SkullMeta)itM).setOwner(data[9]);
	...
	public static ItemStack replace(ItemStack item, String regex, String data) {
		ItemBuilder itemBuilder = new ItemBuilder(item.getType());
		ItemMeta itemMeta = item.getItemMeta();
		List<String> nlore = new ArrayList<>();
		
		itemMeta.setDisplayName(itemMeta.getDisplayName().replace(regex, data));
		
		if(!itemMeta.getLore().isEmpty()) {
			for(String lore : itemMeta.getLore()) {
				String lorez = lore.replace(regex, data);
				nlore.add(lorez);
			}
		}
		itemMeta.setLore(nlore);
		
		itemBuilder.setItMeta(itemMeta);;
