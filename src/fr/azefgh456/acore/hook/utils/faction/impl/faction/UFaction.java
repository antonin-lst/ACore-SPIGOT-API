...
public class UFaction implements IFaction<Faction, FPlayer>{

	private AManager manager;
	
	private Faction faction;
	
	public UFaction(AManager manager, Faction faction) {
	...
	public List<IPlayer<Faction, FPlayer>> getFactionPlayers() {
		...
		faction.getFPlayers().forEach(mPlayer -> {
			factionPlayers.add(new  UPlayer(manager, mPlayer));
		});
	...
	public ChatColor getRelationColor(IFaction<?, ?> target) {
		UFaction ft = (UFaction) target;
		return faction.getColorTo(ft.getFaction());
	}
	
	@Override
	public boolean equals(Object obj) {
		if(obj instanceof UFaction) {
			UFaction target = (UFaction)obj;
