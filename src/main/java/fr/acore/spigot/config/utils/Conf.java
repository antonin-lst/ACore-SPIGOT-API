package fr.acore.spigot.config.utils;
...
public class Conf extends Setupable{
	...
	public static void setStorageType(DatabaseDriver nStorageType) { storageType = nStorageType;}
	//sql !! ajouter le port !! !! rename getDatabase to getDatabaseName == (schema)
	...
	public Conf(ACoreSpigotAPI plugin) {
		super(plugin, true);
	}
	...
	public void setup(FileConfiguration config) {
		...
		storageType = DatabaseDriver.valueOf(config.getString("storage.type"));
